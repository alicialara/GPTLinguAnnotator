...
def save_results(df: pd.DataFrame, output_path: Path, sheet_name: str = None, preserve_all_columns: bool = False) -> None:
    """
    Save results to an Excel file and a CSV file.
    ...
    # Also save to CSV (text format)
    csv_path = output_path.with_suffix('.csv')
    df.to_csv(csv_path, index=False, encoding='utf-8')
    print(f"También se guardaron los resultados en formato texto en: {csv_path}") 