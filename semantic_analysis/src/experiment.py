...
from semantic_analysis.config.config import (
    ...
    ExperimentConfig, AVAILABLE_MODELS, WORD_DICTIONARIES,
...
def run_experiment(experiment: ExperimentConfig, dictionary_key: str, model: str = DEFAULT_MODEL, only_human_annotated: bool = False) -> pd.DataFrame:
    ...
    # Si existe la columna HUMAN, mostrar una evaluación preliminar y guardar anotaciones en JSON y HTML
    if "HUMAN" in df.columns:
        ...
        # Guardar anotaciones en formato JSON y HTML usando el nuevo módulo
        try:
            csv_file_path = output_dir / f"{corpus_name}{suffix}_annotations_{dictionary_key}.csv"
            # Guardar una versión CSV para fácil acceso
            df.to_csv(csv_file_path, index=False)
            
            # Utilizar el nuevo módulo para guardar anotaciones en formato JSON y HTML
            ...
            print(f"Anotaciones guardadas en formato JSON: {json_path}")
            print(f"Anotaciones guardadas en formato HTML: {html_path}")
        except Exception as e:
            print(f"Error al guardar anotaciones en formato JSON/HTML: {e}")
...
def run_all_experiments(model: str = DEFAULT_MODEL, only_human_annotated: bool = False) -> Dict[str, pd.DataFrame]:
    ...
    for experiment in EXPERIMENTS:
        # Obtener los diccionarios correspondientes a esta palabra
        word_dictionaries = WORD_DICTIONARIES.get(experiment.word.lower(), ["GENERIC"])
