...
from semantic_analysis.config.config import DEFAULT_MODEL, EXPERIMENTS, WORD_DICTIONARIES, AVAILABLE_MODELS
    ...
    if args.corpus:
        ...
        if selected_experiments:
            ...
            for experiment in selected_experiments:
                ...
                # Obtener el diccionario correcto para esta palabra
                word_dictionaries = WORD_DICTIONARIES.get(experiment.word.lower(), ["GENERIC"])
                dictionary = word_dictionaries[0]  # Usar el primer diccionario de la lista
                df = run_experiment(experiment, dictionary, model=model, only_human_annotated=args.only_human)
        ...
        if args.word:
            ...
            for experiment in filtered_experiments:
                ...
                dictionary = word_dictionaries[0]  # Usar el primer diccionario de la lista
                df = run_experiment(experiment, dictionary, model=model, only_human_annotated=args.only_human)
                results[f"{model}_{experiment.word}_{experiment.sheet_name}_{dictionary}"] = df
        else:
